...
-   **Learn** how to `mutate()` `across()` multiple variables at once
-   **Learn** how to `summarize()` data with `group_by()` to summarize within categories
-   **Learn** how to `summarize()` data `across()` multiple variables at once.
...
-   [Column-wise operations vignette](https://cran.r-project.org/web/packages/dplyr/vignettes/colwise.html) - more `summarize()` with `across()` examples
